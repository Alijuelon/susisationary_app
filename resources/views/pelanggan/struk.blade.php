...
<head>
    ...
    <title>Invoice Pesanan #{{ $pesanan->id }} - Susi Stationary</title>
    ...
    <div class="max-w-3xl mx-auto bg-white dark:bg-slate-800 p-8 sm:p-12 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-700 print-border relative overflow-hidden transition-colors">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b-2 border-gray-100 dark:border-slate-700 mb-8 gap-4 transition-colors">
            ...
            <div class="text-left sm:text-right">
                ...
                <p class="text-sm font-bold text-gray-800 dark:text-gray-200 mt-1 transition-colors">#ORD-{{ str_pad($pesanan->id, 5, '0', STR_PAD_LEFT) }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 transition-colors">Tgl: {{ \Carbon\Carbon::parse($pesanan->created_at)->locale('id')->translatedFormat('d F Y') }}</p>
            </div>
        </div>
        <div class="mb-8">
            ...
            <p class="text-base font-bold text-gray-800 dark:text-white transition-colors">{{ $pesanan->pelanggan->nama_lengkap }}</p>
